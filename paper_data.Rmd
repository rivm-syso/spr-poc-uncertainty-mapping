...
Fetch Netherlands boundaries from RIVM GIS server. Here as well as in the modelling code, all coordinates are in EPSG:28992 Projected coordinate system for Netherlands. 
...
# This consisted of 24 plots of which the locations seemed to be outside of the Netherlands. The plotnames were found in the LMF plots, and for those the correct coordinates were taken from the LMF programme. 
# There were also an additional 329 abiotic plots with a plot name that was in the LMF file, but where the coordinates did not match. In those cases the coordinates from the LMF programme were taken as well. 
...
# Create data frame
sf.grid <- data.frame( grid.rect[index]) %>% st_as_sf(crs=st_crs(NL))
