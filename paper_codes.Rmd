...
Coordinates are in EPSG:28992, the Projected coordinate system for Netherlands. These coordinates are calculated from the center of the Netherlands in meters. We convert them to kilometers from the center below. 
...
ggsave('results_paper/landuse.tiff', width=1600, heigh=1200, units='px', compression="lzw")
...
ggsave('results_paper/landuse_vs_visit.tiff', width=1400, heigh=900, units='px', compression="lzw")
```
...
ggplot(NL) + geom_sf() +
	...
	geom_sf(aes(geometry=geometry, color=has_abiotics), shape=4, size=0.5, 
	        data=sf.plots %>% arrange(has_abiotics)) + 
	...
	scale_colour_manual(values=c("#377EB8", "#4DAF4A"), name="Abiotic measurements") +
  scale_fill_discrete(guide=guide_legend(reverse=T))
	#ggtitle('Location of Field Visits and Abiotics') 
ggsave('results_paper/visits.png', width=1400, heigh=1200, units='px')
ggsave('results_paper/visits.tiff', width=1400, heigh=1200, units='px', compression="lzw")
```
...
ggsave('results_paper/Struikhei.tiff', width=2000, heigh=1400, units='px', compression="lzw")
```
...
plot.all
ggsave('results_paper/pH.png', width=2000, heigh=1400, units='px')
ggsave('results_paper/pH.tiff', width=2000, heigh=1400, units='px', compression="lzw")
```
...
ggsave('results_paper/pH2.tiff', width=3000, heigh=1400, units='px', compression="lzw")
```
...
Place the data in the subfolder `data_paper/` and run the code below this cell. 
...
```{r}
# Linear terms
labels <- c("Organic Material","Carbon/Nitrogen","Nitrogen","Phosphor","Potassium")
...
plot.abiotic <- ggplot(terms.plot, aes(x=ID, y=mean, col=meta, group=meta)) + 
  ...
  scale_x_discrete(name='Covariate',labels= labels) + 
...
terms.plot <- terms %>% filter((term =="spatial") & (model != "SDM")) %>%
  ...
  mutate(term=recode(term, "spatial" = "Spatial (km)"))%>%
  mutate(meta=factor(recode(meta, "Certain" = "Two-Stage", "Uncertain"="Joint"), levels=c("Two-Stage", "Joint"))) %>%
  mutate(model=recode(model, "ORG_STOF.z"="Organic Material","CN.z"="Carbon/Nitrogen","N_totaal.z"="Nitrogen","P_totaal.z"="Phosphor","K_CaCl2.z"="Potassium"))
...
ggsave('results_paper/species_parameters.tiff', width=2000, heigh=2500, units='px', compression="lzw")
```
...
ggsave('results_paper/species_SDM_y.tiff', width=3200, heigh=3000, units='px', compression="lzw")
```
...
ggsave('results_paper/species_SDM_logodds.tiff', width=3200, heigh=3000, units='px', compression="lzw")
```
...
ggsave(sprintf('results_paper/species_%s.tiff', var), width=3200, heigh=3000, units='px', compression="lzw")
}
```
...
ggsave('results_paper/species_scatter.tiff', width=1800, heigh=2400, units='px', compression="lzw")
```
...
ggsave('results_paper/all_abiotics.tiff', width=2400, heigh=4000, units='px', compression="lzw")
```
...
ggsave('results_paper/species_correlation.tiff', width=2000, heigh=1200, units='px', compression="lzw")
```
...
levels(sf.grid$fgr) <- list("Closed estuaries"="afz",
                         "Dunes mainland"="duo",
                         "Dunes Wadden islands"="duw",
                         "Tidal areas Wadden islands"="gtw",
                         "Tidal areas Zeeland"="gtz",
                         "Hill country Limburg"="hll",
                         "Higher sandy soils North"="hzn",
                         "Higher sandy soils East"="hzo",
                         "Higher sandy soils Veluwe"="hzv",
                         "Higher sandy soils South"="hzz",
                         "Low peat Holland"="lvh",
                         "Low peat North"="lvn",
                         "River area"="riv",
                         "Sea clay middle"="zkm",
                         "Sea clay North"="zkn",
...
colors = c("Closed estuaries"="#d62627",
           "Dunes mainland"="#ea6d6e",
           "Dunes Wadden islands"="#ffbfbf",
           "Tidal areas Wadden islands"="#1f77b4",
           "Tidal areas Zeeland"="#64a9d9",
           "Hill country Limburg"="#bfe4ff",
           "Higher sandy soils North"="#ff7f0e",
           "Higher sandy soils East"="#ffae66",
           "Higher sandy soils Veluwe"="#bfffbf",
           "Higher sandy soils South"="#ffddbf",
           "Low peat Holland"="#6acf6a",
           "Low peat North"="#2ca02c",
           "River area"="#00BFC4",
           "Sea clay middle"="#b88fde",
           "Sea clay North"="#9466bd",
           "Sea clay South"="#e0bfff")

ggplot(NL) + geom_sf() +
	geom_sf(aes(geometry=geometry.1, fill=fgr,linewidth=0.1,colour=""), data=sf.grid) +
  scale_linewidth_identity() +
  scale_fill_manual(values=colors, na.value='gray',na.translate = F,name='Physical Geological Region') +
  scale_colour_manual(values=NA) +
  guides(colour=guide_legend("Not mappable", override.aes=list(colour="gray"))) +
	theme(legend.position="right") + ggtitle('Physical Geographical Regions') 
ggsave('results_paper/fgr.png', width=2000, heigh=1400, units='px')
ggsave('results_paper/fgr.tiff', width=2000, heigh=1400, units='px', compression="lzw")

```

```{r}
prov_geom <- read_sf("data_paper/provincies.gpkg")$geom
prov_geom <- (prov_geom - matrix(data=c(155000, 463000), ncol=2)) * matrix(data=c(1/1000, 1/1000), ncol=2)
colors = c("#1f77b4","#aec7e8",
          "#ff7f0e","#ffba78",
          "#2ca02c","#98df8a",
          "#d62627","#ff9896",
          "#9466bd","#c5b0d5",
          "#8c564b","#c49c94")
names(colors) = c('Drenthe',
 'Flevoland',
 'Fryslân',
 'Gelderland',
 'Groningen',
 ...
 'Zuid-Holland')
ggplot() + 
	geom_sf(aes(geometry=geometry.1, fill=naam,linewidth=0.1), data=sf.grid) +
  geom_sf(aes(geometry=prov_geom),fill=NA) +
  scale_linewidth_identity() +
  scale_fill_manual(values=colors, na.value='gray',name='Province') +
  theme(legend.position="right") +ggtitle('Provinces of the Netherlands')  
ggsave('results_paper/provinces.png', width=1700, heigh=1200, units='px')
ggsave('results_paper/provinces.tiff', width=1700, heigh=1200, units='px', compression="lzw")
...
ggsave('results_paper/leave_out_validation.tiff', width=2500, heigh=1300, units='px', compression="lzw")
```
...
ggsave('results_paper/rmse_comparison.tiff', width=1200, heigh=800, units='px', compression="lzw")
```
...
prevalences.both <- rbind(prevalences.predicted, prevalences.observed) %>%
  ...
  mutate(naam=recode(naam, "Drenthe"='DR', "Flevoland"='FL', "Fryslân"='FR', "Gelderland"='GE', "Groningen"='GR',
...
ggsave('results_paper/all_prevalences.tiff', width=2800, heigh=4000, units='px', compression="lzw")
```
...
ggsave('results_paper/all_prevalences2.tiff', width=2800, heigh=4000, units='px', compression="lzw")
```
