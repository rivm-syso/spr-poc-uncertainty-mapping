...
sf.train = sf.plots %>% filter(Fys_geo_regio != geo) #%>% filter(bodemgebruik %in% c("Bos", "Droog natuurlijk terrein", "Nat natuurlijk terrein"))
sf.test = sf.plots %>% filter(Fys_geo_regio == geo) #%>% filter(bodemgebruik %in% c("Bos", "Droog natuurlijk terrein", "Nat natuurlijk terrein"))
...
if (!is.null(output)) {
    ...
    predictions <- cbind(predicted, observed) %>% mutate(meta = "Uncertain", model="SDM", species=species.take, naam = geo)
    prevalences <- data.frame(area_prevalences, meta = "Uncertain", model="SDM", species=species.take, naam = geo)

    fn.1 <- sprintf("/mnt/scratch_dir/viljanem/predictions_uncertain2/%s_%s.csv", species.take, geo)
    fn.2 <- sprintf("/mnt/scratch_dir/viljanem/prevalences_uncertain2/%s_%s.csv", species.take, geo)
