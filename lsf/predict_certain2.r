...
if (!is.null(output)) {
    ...
    predictions <- cbind(predicted, observed) %>% mutate(meta = "Certain", model="SDM", species=species.take, naam = geo)
    prevalences <- data.frame(area_prevalences, meta = "Certain", model="SDM", species=species.take, naam = geo)

    fn <- sprintf("/mnt/scratch_dir/viljanem/predictions_certain2/%s_%s.csv", species.take, geo)
    write.table(predictions, fn, sep = ",", col.names = F, row.names=F, append=F)
    fn <- sprintf("/mnt/scratch_dir/viljanem/prevalences_certain2/%s_%s.csv", species.take, geo)
